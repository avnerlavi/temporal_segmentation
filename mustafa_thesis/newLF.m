% %code of the LF with steerable and new version of LF:
% IM=imread('C:\Users\97254\Pictures\synth.PNG');
% I=im2double(rgb2gray(IM));
% %add noise:
% Noisy=imnoise(I,'speckle',0.01);
% figure;imshow(Noisy,[])%%% imopen
% 
% %med=medfilt2(m,[2 2]); %LPF anistropic diff
% [gradThresh,numIter] = imdiffuseest(Noisy,'Connectivity','minimal');%'ConductionMethod','quadratic');
% m = imdiffusefilt(Noisy,'ConductionMethod','quadratic', ...
%     'GradientThreshold',gradThresh,'NumberOfIterations',numIter);

%%
%close all;clear all;
tic
%IMG = imread('C:\Users\97254\Documents\MATLAB\thesis\Images\ShoulderPartialTear1.jpg');
%IMG=imread('girl.PNG');
IMG=im2double(I);
if size(IMG,3)>1
    IMGhsv=rgb2hsv(IMG);
    img=IMGhsv(:,:,3);   
else
    img=im2double(IMG);
end
SS=4;
IMscel=zeros(size(img,1),size(img,2),SS);
IMsceltot=zeros(size(img,1),size(img,2));
for j=1:SS
    
    RSfactorR = round(size(img,1)/max(1,2*(j-1)));
    RSfactorC = round(size(img,2)/max(1,2*(j-1)));


    imgS=imresize(img,[RSfactorR RSfactorC]);
    theta=0:15:90;
    KK=length(theta);
    
    LFimmNor=zeros(size(imgS,1),size(imgS,2),KK);
    LFimmPor=zeros(size(imgS,1),size(imgS,2),KK);
    immNor=zeros(size(imgS,1),size(imgS,2));
    immPor=zeros(size(imgS,1),size(imgS,2));
    
    J1=zeros(size(imgS,1),size(imgS,2),length(theta));
    J2=zeros(size(imgS,1),size(imgS,2),length(theta));
    J=zeros(size(imgS,1),size(imgS,2),length(theta));
    P=zeros(size(imgS,1),size(imgS,2),length(theta));
    N=zeros(size(imgS,1),size(imgS,2),length(theta));

    sigma=1;
    
    for i =1:KK
        
%         tat=pi()*(i-1)/KK;
%         x=1:25;
%         y=1:25;
%         x0=13;
%         y0=13;
%         sig=8;
%         lmd=12;
% 
%         exp1=0;
%         L = zeros(size(y,2),size(x,2));
%         Lnorm = zeros(size(y,2),size(x,2));
    %         
%         for x1=1:size(x,2)
%             for y1=1:size(y,2)
%                 exp1=exp(-((x(x1)-x0)^2/sig^2+(y(y1)-y0).^2/(sig)^2));
%                 L(y1,x1)=cos(2*pi/lmd*((x(x1)-x0)*cos(tat)+(y(y1)-y0)*sin(tat))).*exp1;
%                 Lnorm(y1,x1)=cos(2*pi/lmd*((x(x1)-x0)*cos(tat)+(y(y1)-y0)*sin(tat)));
%             end
%         end
% 
%         Lnorm=imresize(Lnorm,1/5);
%         L=imresize(L,1/5);
% 
%         Lnorm  =conv2(Lnorm,L,'same');
%         CONVnorm = Lnorm(ceil(size(L,1)/2),ceil(size(L,1)/2));
%         imgO = conv2(imgS,L,'same')/CONVnorm; %THR=0.009
%         imgP = max(imgO,0);
%         imgN = max(-imgO,0);
%         
        J1(:,:,i) = steerGaussFilterOrder1(imgS,theta(i),sigma,true);
        J2(:,:,i) = steerGaussFilterOrder2(imgS,theta(i),sigma,true);
        J(:,:,i)=J1(:,:,i);
        P(:,:,i)=max(J(:,:,i),0);
        N(:,:,i)=max(-J(:,:,i),0);
%         J=imgO;
%         P=imgP;
%         N=imgN;
        
        THR(i)=max(max(abs(J(:,:,i))));
        P=(P(:,:,i)>0.01*THR(i)).*P(:,:,i);
        N=(N(:,:,i)>0.01*THR(i)).*N(:,:,i);

        %Fac=max(1,10./j);
        Fac=1;
        tat=theta(i)/180;
        [LFimmN NimNR] = LFsc(N,tat,Fac);%change the LFsc func
        [LFimmP PimNR] = LFsc(P,tat,Fac);
        LFimmP=LFimmP./max(LFimmP(:));
        LFimmN=LFimmN./max(LFimmN(:));

        LFimmN=0.5*max(0,LFimmN-0.1*NimNR);
        LFimmP=0.5*max(0,LFimmP-0.1*PimNR);

        immNor=immNor+LFimmN;
        immPor=immPor+LFimmP;
%     figure;imshow(immPor,[])
    end
    IM2scel =immPor-immNor;
    
    IMscel(:,:,j)=imresize(IM2scel,[size(img,1) size(img,2)]); %();

    IMscel(:,:,j) = IMscel(:,:,j)/2.^(j-1);
%     if (j>1)
%         IMscel(:,:,j)=IMscel(:,:,j).^0.8;
%     end


%      figure; imagesc(imgS);colormap gray
% % 
%     figure; imagesc(min(1,max(0,(imgS-immNor/2+immPor/8))));colormap gray
     IMsceltot=IMsceltot+IMscel(:,:,j);
end
IMsceltot1=IMsceltot; 
%IMsceltot1=IMsceltot/(KK*SS.^0.5);
% IMsceltot1=0.3*sign(IMsceltot).*(min(abs(IMsceltot),0.5)).^0.5;
% C=1/max(IMsceltot(:));
IMsceltot1=0.1*sign(IMsceltot).*(min(abs(IMsceltot),5)).^0.5;
tmp=img+IMsceltot1;
outF=img+(IMsceltot1>0).*IMsceltot1./max(1,15*(tmp-0.7).^2)-(IMsceltot1<0).*IMsceltot1./(1+5*(tmp<0.1).*(0.1-tmp));%min(0,10*(tmp)))

outFin=min(1,max(0,outF));
figure;subplot(1,2,1);imshow(IMG,[]);subplot(1,2,2);imshow(outFin,[])
%%
IMsceltot_shift=IMsceltot+(abs(min(IMsceltot(:))));
IMsceltot_norm=IMsceltot_shift./max(IMsceltot_shift(:));
M=imadjust(img,[],[0 1]);
Ifinal=M-IMsceltot_norm;
D=(I+abs(min(I(:))));
IMGhsv(:,:,3)=double(D);
A=hsv2rgb(IMGhsv); 
outColor=A;figure;imshow(outColor,[])
