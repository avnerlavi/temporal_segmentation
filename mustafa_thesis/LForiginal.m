function [out,IMsceltot1]=LForiginal(inImg)
% close all;
%clear all;
...
%           imgN=(imgN>0.05*THR).*imgN;
 ...
 %%       
%          cnP=zeros(size(imgS,1),size(imgS,2));          
%          cnN=zeros(size(imgS,1),size(imgS,2));
%          for nn=2:KK-1
%              if (nn ~= i)
%                  
%                  tat=pi()*(i-1)/KK;
...
% 
%                  exp1=0;
%                  L2 = zeros(size(y,2),size(x,2));
%                  Lnorm2 = zeros(size(y,2),size(x,2));
%         %         
%                  for x1=1:size(x,2)
%                      for y1=1:size(y,2)
%                          exp1=exp(-((x(x1)-x0)^2/sig^2+(y(y1)-y0).^2/(sig)^2));
%                          L2(y1,x1)=cos(2*pi/lmd*((x(x1)-x0)*cos(tat)+(y(y1)-y0)*sin(tat))).*exp1;
%                          Lnorm2(y1,x1)=cos(2*pi/lmd*((x(x1)-x0)*cos(tat)+(y(y1)-y0)*sin(tat)));
%                      end
%                  end
% 
%                  Lnorm2=imresize(Lnorm2,1/5);
%                  L2=imresize(L2,1/5);
% 
%                  Lnorm2  =conv2(Lnorm2,L2,'same');
%                  CONVnorm2 = Lnorm2(ceil(size(L2,1)/2),ceil(size(L2,1)/2));
%                  imgO2 = conv2(imgS,L2,'same')/CONVnorm2; %THR=0.009
%                  imgP2 = max(imgO2,0);
%                  imgN2 = max(-imgO2,0);
%                  cnP=cnP+imgP2;
%                  cnN=cnN+imgN2;
...
%          end
          ...
          LFimmN1=0.5*max(0,LFimmN1-0.2*NimNR1);
          LFimmP1=0.5*max(0,LFimmP1-0.2*PimNR1);      
...
%           immNor=max(immNor1,immNor2);
%           immPor=max(immPor1,immPor2);

          %IM2scel =-LFimmN2-LFimmN1+immPor1+immPor1;
          %figure;imshow(IM2scel,[]);
         
          
     end
    IM2scel =immPor1-immNor1;
    %IM2scel =immPor1;
    %IMscel=newFunc(IM2scel,'spline');
    IMscel=imresize(IM2scel,[size(I,1) size(I,2)]); %reconstruction ;
    %%
%     lw=256;
%     lh=256;
%     [X,Y]       =   meshgrid(1:1/j:lw, 1:1/j:lh);
...
%IMsceltot1=0.3*min(0.2*IMsceltot,1);%IMsceltot/(KK*SS.^0.5);
C=1/max(IMsceltot(:));
