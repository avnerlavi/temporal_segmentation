function AlgParams = SetAlgorithmParams2(Path,ImgNum,Format)
...
AlgParams.InputImgBmodeOld = imread([Path],AlgParams.ImgType);
IM=imread([Path],AlgParams.ImgType);
%J = adapthisteq(rgb2gray(IM),'clipLimit',0.02,'Distribution','rayleigh');
J = adapthisteq(IM,'clipLimit',0.02,'Distribution','rayleigh');

AlgParams.InputImgBmode=imresize(J,[256 256]);
...
AlgParams.CenSizes.Bmode =             [7 9 11];%[20 25 35 40];   %[3 5 7 10 12 15 18 20]; %5:5:35; % 20:10:40; % center area resolutions for B-mode image
