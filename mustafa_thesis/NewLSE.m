...
ROI = (Img>0); ROI = double(ROI);

tic

Bas=getBasisOrder3(nrow,ncol);
N_bas=size(Bas,N_region);
for ii=1:N_bas
...
energy_MICO = zeros(N_region,iterNum);

b=ones(size(Img));
for ini_num = 1:1
    C=rand(N_region,1);
    C=C*A;
    M=rand(nrow,ncol,N_region);
    a=sum(M,3);
    for k = 1 : N_region
        M(:,:,k)=M(:,:,k)./a;
    end
    
    [e_max,N_max] = max(M,[],N_region);
    for kk=1:size(M,N_region)
        M(:,:,kk) = (N_max == kk);
    end
