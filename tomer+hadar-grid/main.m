...
if (dump_movies)
    aviobj = VideoWriter(['new\movie_center_mass_',num2str(block_size),'.avi']);
    aviobj.Quality = 80;
    open(aviobj);
...
if (dump_movies)
    aviobj = VideoWriter(['new\movie_std_',num2str(block_size),'.avi']);
    aviobj.Quality = 80;
    open(aviobj);
    for i =1:size(Msdt,3)
...
if (dump_movies)
    aviobj = VideoWriter(['new\movie_std_cc_obj_',num2str(block_size),'.avi']);
    aviobj.Quality = 80;
    open(aviobj);
...
if (dump_movies)
    aviobj = VideoWriter(['new\movie_prob_map_',num2str(block_size),'.avi']);
    aviobj.Quality = 80;
    open(aviobj);
...
% % % % % % % % % % % % % % % % % % % % % %
aviobj = VideoWriter(['new\movie_final_w_cc_obj_',num2str(block_size),'.avi']);
aviobj.Quality = 80;
open(aviobj);
for i =1:15
...
%    Combine all results to Final movie   %
% % % % % % % % % % % % % % % % % % % % % %
aviobj = VideoWriter(['new\movie_final_w_prob_map_',num2str(block_size),'.avi']);
aviobj.Quality = 80;
open(aviobj);
for i =1:15
   writeVideo(aviobj,uint8(vid_matrix(:,:,i))*255);   
